...
from cxl_ecosys.util.unaligned_bit_structure import BitMaskedBitStructure
from cxl_ecosys.util.number import round_up_to_power_of_2
from cxl_ecosys.pci.component.fifo_pair import FifoPair
from cxl_ecosys.pci.component.packet_processor import PacketProcessor
...
class MmioManager(PacketProcessor):
    ...
    async def _send_completion(self, req_id: int, tag: int, data: int = None):
        if data is not None:
    ...
    async def _process_mmio_packet(self, mem_req_packet: CxlIoMemReqPacket):
        address = mem_req_packet.mreq_header.addr
        size = mem_req_packet.cxl_io_header.length
        req_id = mem_req_packet.mreq_header.req_id
        ...
        if mem_req_packet.is_mem_write():
            ...
            await self._send_completion(req_id, tag, data)
        else:
