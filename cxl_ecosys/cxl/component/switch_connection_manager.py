...
from cxl_ecosys.cxl.component.packet_reader import PacketReader
from cxl_ecosys.cxl.component.cxl_packet_processor import CxlPacketProcessor
    ...
    async def _run(self):
        ...
        for port_index, port in enumerate(self._ports):
            if port.packet_processor is not None:
                logger.info(
                    self._create_message(f"Stopping PacketProcessor for port {port_index}")
                )
