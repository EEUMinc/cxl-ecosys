...
from cxl_ecosys.cxl.component.connection_client import (
    ConnectionClient,
    INJECTED_ERRORS,
)
...
from cxl_ecosys.util.pci import create_bdf
...
async def test_switch_connection_manager_handle_connection():
    ...
    async def wait_and_connect():
        ...
        logger.info("[PyTest] Starting ConnectionClient")
        await client.run()

    async def stop():
        await client.wait_for_ready()
        logger.info("[PyTest] ConnectionClient is ready")
        logger.info("[PyTest] Stopping SwitchConnectionManager")
        await manager.stop()
        logger.info("[PyTest] Stopping ConnectionClient")
        await client.stop()
